...
### sudo ./lxde.sh komutu ile çalıştırınız
...
chroot kaynak apt-get install blueman gvfs-backends neofetch gnome-calculator synaptic rar -y

### Pardus paketleri kuralım 
chroot kaynak apt-get install pardus-common-desktop pardus-configure pardus-locales -y
chroot kaynak apt-get install pardus-image-writer pardus-package-installer pardus-power-manager pardus-software pardus-usb-formatter pardus-welcome -y
chroot kaynak apt-get install pardus-dolunay-grub-theme pardus-gtk-theme pardus-icon-theme pardus-backgrounds pardus-about pardus-installer -y
...
grub-mkrescue isowork -o pardus-lxde-live-$(date +%x).iso
